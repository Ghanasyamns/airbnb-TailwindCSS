...
function CardSection(props) {
  ...
  const singleCard = props.city?.map(
    ({ imageUrl, pricePerNight, rating, title, location, id }, i) => {
      return (
        <Link key={i} to={`/venue/${id ?? 14}`}>
          <div
            onMouseOver={() => context.update(i)}
            className="relative  flex flex-col md:py-4 md:flex-row md:border-b-[1px] border-gray-200 "
          >
            ...
            <div className="relative  my-4 md:ml-5 flex-grow">
              <div className="flex space-x-1 md:absolute bottom-0 left-0">
                ...
                <h1 className="md:font-bold">{rating} </h1>
            ...
            >
              <HeartIcon
                id="fav-icon"
                className=" absolute  top-0 right-0 w-7  h-7 mt-3 mr-3 text-white
             active:scale-110 transition transform duration-150 ease-in  md:hidden"
              />

              <HeartIcon
                id="fav-icon"
                className="hidden md:inline absolute top-3 right-0 w-7 h-7 mt-3 mr-3 
  ...
  return (
    <div onMouseLeave={() => context.update(null)} className="font-card">
      {singleCard}
    </div>
  );
}
